            ...
            _paraDic.forEach([this,ptrMsg,&bResult](const SCString& strKey,const SCValue& value) -> bool
                             {
                                 ...
                                 do
                                 {
                                     ...
                                     {
                                         if((value2.getType()==SCValue::FLOAT_TYPE || value2.getType()==SCValue::DOUBLE_TYPE) &&
                                            (value.getType()==SCValue::FLOAT_TYPE || value.getType()==SCValue::DOUBLE_TYPE))
                                         {
                                     ...
                                     {
                                         if(value2.getType()==SCValue::BOOL_TYPE)
                                         {
                                             bResult = (value2.getBool()==value.getBool());
                                         }
                                         else if(value2.getType()==SCValue::STRING_TYPE)
                                         {
